...
class SitekeysDB:
    ...
    def add(self, sitekey: str, url: str, platform_name: str = None, status: str = "unknown", tags: List[str] = None) -> SitekeyEntry:
    ...
    def export_markdown(self) -> str:
        ...
        lines = [
            "# Alap-Alap Captcha Database",
            "",
            "Community-maintained list of discovered Cloudflare Turnstile sitekeys and solved tokens.",
            "",
        ...
        for entry in sorted(self._data.values(), key=lambda x: x.platform_name):
            ...
            lines.append(
                f"| **{entry.platform_name}** | `{entry.sitekey[:20]}...` | {token_display} | {status_icon} {entry.status} | {solve_time} | {rate} |"
            )
